...
let readLines = String.splitLines >> Array.filter (_.Length >> (<) 0) >> array2D
...
let checkMask word (inBounds, get) pos dir =
    ...
    word |> Seq.mapi step |> Seq.forall id

let countDirections api pos =
    dirs |> Array.sumBy ((checkMask "XMAS" api pos) >> zeroOne)
...
let solution1 data =
    let api = data |> api
    data |> Array2D.mapi (fun x y _ -> countDirections api (x, y)) |> sum

let solution2 data =
    let api = data |> api

    data
    |> windowFold (fun s (_, x1, y1, _, _) -> (x1, y1) |> checkMas api |> zeroOne |> (+) s) 0 (3, 3)

let data = loadSampleData "Puzzle2.txt" |> readLines
