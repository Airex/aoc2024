...
let inline stepBack dir pos =
    pos |> add (dir |> dirToPair |> opposite)
    ...
    | false ->
        ...
        | true ->
            ...
            | c when c = '.' || isGuard c -> // we are at empty cell
                let cycled =
                    let inline canDoCycle pos =
                        [ Up; Down; Left; Right ]
                        |> List.exists (fun x -> visited |> HashSet.contains (pos, x))
                        |> not
                ...
                stepper
                    ...
                    (pos |> step dir)
                    dir
                    (visited |> HashSet.add (pos, dir))
                    (cyclesCount + (cycled |> zeroOne))
...
let solution1 data =
    ...
    let p1 =
        path
        |> HashSet.fold (fun acc x -> x |> fst |> flip HashSet.add acc) HashSet.empty
        |> HashSet.count
