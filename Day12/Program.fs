...
let unpackBySide side v =
    v
    |> List.unzip
    |> snd
    |> List.unzip
    |> (if leftOrRightSide side then fst else snd)

let hasGap (a, b) = abs (a - b) > 1

let gapsCount lst =
    lst |> Seq.sort |> Seq.pairwise |> Seq.sumBy (hasGap >> zeroOne)

let calculateFencePrice discountFn data =
    let inBounds, get = data |> api

    let (|PlantOfDifferentKind|_|) plant x =
        if (inBounds x |> not) || get x <> plant then
            Some x
        else
            None
...
let sidesDiscount sides =
    let sideOnSameRowOrCol (side, pos) =
        if leftOrRightSide side then
            (side, snd pos)
        else
            (side, fst pos)

    let numberOfConnectedSidesOnSameRowOrColumn ((side, _), v) =
        v |> unpackBySide side |> gapsCount |> (+) 1
