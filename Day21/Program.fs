...
type Pad(value: char array2d) =
    ...
    // less turns are the better, as we do not move to other button = just A on next layer
...
let solve layersCount input =
    ...
    let rec find code layer =
        ...
        match layer with
        | l when l = layersCount + 1 -> code |> String.length |> int64
        | _ ->
            match cache[layer].TryGetValue(code) with
            | true, v -> v
            | _ ->
                let len = iterate code layer
                cache[layer].Add(code, len)
                len
